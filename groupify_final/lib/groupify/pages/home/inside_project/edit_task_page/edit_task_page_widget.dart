...
class EditTaskPageWidget extends StatefulWidget {
  ...
  final String? tDue;
  final String? subtaskflag;
  const EditTaskPageWidget({super.key, this.projectName, this.pOwnerId, this.pDescription, this.tName, this.tDescription, this.tProgress, this.tDifficulty, this.tAssigned, this.tDue, this.subtaskflag});
...
Future<void> _updateTask(double newtProgress) async {
  ...
  if (_model.datePicked == null){
    ...
    newtDue = '$month/$day/$year';
  }

final String newtDescription = _model.taskDescriptionController.text;
  final double? newtDifficulty;
  if (_model.ratingBarValue == null) {
    newtDifficulty = double.parse(widget.tDifficulty ?? '0.0');
} else {
    newtDifficulty = _model.ratingBarValue;
}
  final List<String>? taskAssigned = _model.dropDownValueController?.value;
  final String newtAssigned = taskAssigned?.join(', ') ?? widget.tAssigned.toString();
...
Future<void> _updateTask2() async {
  String? newtDue;
  if (_model.datePicked == null){
    newtDue = widget.tDue.toString();
  } else {
...
final String newtDescription = _model.taskDescriptionController.text;
  ...
  try{
    final results = await _sqldatabaseHelper.connection.query( 
        'update Tasks set taskDescription = ?, taskDifficulty = ?, taskAssigned = ?, taskDueDate = ? where projectName = ? and ownerID = ? and taskName = ?;',
            [newtDescription, newtDifficulty, newtAssigned, newtDue, widget.projectName, widget.pOwnerId, widget.tName]);
  ...
  Widget taskwsubtask(BuildContext context){
  ...
  Widget taskwnosubtask(BuildContext context){
    List<String> selectedValue = [];
 ...
 return Column(
  ...
  Widget build(BuildContext context) {
    List<String> selectedValue = [];
    bool flag = bool.parse(widget.subtaskflag ?? '');
                ...
                child: Container(
                  ...
                  child: Padding(
                    padding:
                      ...
                      children: [
                        if(flag == false)
                          taskwnosubtask(context)
                        else 
                          taskwsubtask(context)
                    ]
          ),
                ),
              ),
