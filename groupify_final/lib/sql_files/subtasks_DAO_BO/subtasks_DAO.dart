...
class Subtasks_DAO{
  ...
  Future<List<SubTask>> getSubTasks(projectName, projectOwnerID, taskName) async {
    final SQLDatabaseHelper _sqldatabaseHelperSub = SQLDatabaseHelper(); // Initialize database connectiong helper
    await _sqldatabaseHelperSub.connectToDatabase();
    List<SubTask> subtasks = [];
    try{
      final results = await _sqldatabaseHelperSub.connection.query('select taskName, subTaskName, subTaskDescription, subTaskProgress, subTaskDifficulty, subTaskAssigned, subTaskDueDate from Subtasks where projectName = ? and ownerID = ? and taskName = ?;',
      ...
      for(final row in results){
        ...
        for(var split in splitted){
          ...
          sum = 0;
          split = split.trim();
          final results2 = await _sqldatabaseHelperSub.connection.query('SELECT rating FROM userRating WHERE userID = ?;',
                                                              [split.toString()]); 
    ...
    await _sqldatabaseHelperSub.closeConnection();
    return subtasks;
  }
}